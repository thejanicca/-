...
def start(message):
    ...
    user_data[message.from_user.id] = {"current_book": None,
                                     "position": 0}
    ...
    bot.send_message(
        message.chat.id,
        "Добро пожаловать в книжного бота! Я помогу вам читать "
        "книги по частям, а также находить определения неизвестных "
        "английских слов и подбирать к ним синонимы.\n\n"
...
def handle_document(message):
    ...
    if not message.document.file_name.endswith('.txt'):
        bot.reply_to(message, "Пожалуйста, отправьте файл в "
                             "формате TXT.")
        return

    # Скачиваем файл
    file_info = bot.get_file(message.document.file_id)
    file_path = os.path.join(BOOKS_DIR,
...
def start_reading(message):
    ...
    if user_id not in user_data or not user_data[user_id]["current_book"]:
        bot.send_message(user_id, "У вас нет текущей книги. "
...
def restart_reading(message):
    ...
    bot.send_message(user_id, "Готово! Теперь вы можете отправить "
                            "новую книгу.")
...
def define_word(message):
    ...
    msg = bot.send_message(message.from_user.id,
                         "Пожалуйста, отправьте слово для его определения",
...
def send_next_chunk(user_id):
    ...
    if not book_data["current_book"] or not os.path.exists(book_data["current_book"]):
        bot.send_message(user_id, "Книга не найдена. Пожалуйста, "
...
def give_definition(message):
    ...
    if not synsets:
        bot.send_message(message.chat.id, "Извините, я не смогу дать "
    ...
    try:
        ...
        pass  # Слово не найдено в списке - отсутствие действий
...
def reminder(message):
    ...
    if used is False:
        bot.send_message(message.chat.id,
                        "Экологичное напоминание продолжить "
                        "читать книгу:)")
    else:
        used = False
